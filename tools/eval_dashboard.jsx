...
const ReactRuntime = RuntimeScope.React
const ReactDOMRuntime = RuntimeScope.ReactDOM
const RechartsRuntime = RuntimeScope.Recharts

if (!ReactRuntime) {
  throw new Error("eval_dashboard.jsx requires a global React runtime.")
}

if (!RechartsRuntime) {
  throw new Error("eval_dashboard.jsx requires a global Recharts runtime.")
}

const { useEffect, useState } = ReactRuntime
const {
  Bar,
  BarChart,
  CartesianGrid,
...
function EvalDashboard() {
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_top,_rgba(15,118,110,0.14),_transparent_36%),linear-gradient(180deg,_#f8fafc_0%,_#eef2ff_100%)] text-slate-900">
      <div className="mx-auto flex max-w-7xl flex-col gap-6 px-4 py-6 sm:px-6 lg:px-8">
        <header className="rounded-[2rem] border border-slate-200 bg-white/85 px-6 py-6 shadow-sm shadow-slate-200/70 backdrop-blur">
...
if (typeof window !== "undefined") {
  ...
  if (rootNode && ReactDOMRuntime && !rootNode.dataset.evalDashboardMounted) {
    rootNode.dataset.evalDashboardMounted = "true"
    if (typeof ReactDOMRuntime.createRoot === "function") {
      ReactDOMRuntime.createRoot(rootNode).render(<EvalDashboard />)
    } else if (typeof ReactDOMRuntime.render === "function") {
      ReactDOMRuntime.render(<EvalDashboard />, rootNode)
    }
  }
}
